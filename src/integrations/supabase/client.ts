...
export const supabase = createClient<Database>(
  ...
  SUPABASE_PUBLISHABLE_KEY,
  {
    ...
    // Fix for TypeScript error by using correct approach without type assertion
    db: {
      schema: 'public' as any, // Using 'as any' to bypass the TypeScript restriction
    },
  }
);
...
const checkConnection = async () => {
  try {
    ...
    // Try creating an RPC function to test connection if it doesn't exist already
    try {
      await supabase.rpc('ping').select('*');
    } catch (e) {
      console.log('Ping function may not exist, using profiles table instead');
    }
